...
int binary_cws_simulation(RDATA *rdata, S_LIST *sl, int probability) { // TODO: 確率を範囲で指定する場合をどうするか
	...
	*/
	if (sl->num_enable_sav == 0) { // エラー処理
		printf("num_sav_cpy is zero.\n");
		exit(1);
	}
	i=0;
	while (sl->num_enable_sav > 0) {
...
int search_best_savings(S_LIST *sl, RDATA *rdata) {
   ...
   */
	...
	for (i=0; i < loop_max; i++) {
		...
		sl_copy->num_enable_sav = sl->num_enable_sav;
		...
		sl->num_enable_sav = sl_copy->num_enable_sav;
...
void mcts_cws(S_LIST *sl, RDATA *rdata) {
	int best_savings_idx, rest;
	S_LIST *sl_cpy;
	...
	while (0 < sl->num_sav) {
		...
		sl_cpy->num_enable_sav = sl->num_enable_sav;
		memcpy(sl_cpy->edge, sl->edge, sizeof(EDGE)*vrp->edgenum);

		best_savings_idx = search_best_savings(sl_cpy, rdata); // 最も良いsavings listのインデックスを取得
		printf("best_savings_idx:%d\n", best_savings_idx);
		if (best_savings_idx != -1) {
